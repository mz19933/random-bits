...
# Adding more controllers or nodes
# We will need 3 things, base on this command - kubeadm join <control-plane-host>:<port> --token <token> --discovery-token-ca-cert-hash <hash>
# <control-plane-host>:<port> | our main controller ip\dns with port (default is 6443)
# --token <token> | kubeadm token create --ttl 24h (good practice to limit it live time, we also only need it for this phase)
# --discovery-token-ca-cert-hash <hash> | openssl x509 -pubkey -in /etc/kubernetes/pki/ca.crt | openssl rsa -pubin -outform der 2>/dev/null | openssl dgst -sha256 -hex | sed 's/^.* //'
